...
- **Language-Tutor Avatars** — whether an avatar teaches is decided by the reference backend from the *content* of its `persona` text (a keyword check for teaching-flavored language), not a flag in `AvatarSources.js`. When the backend detects a teaching persona **and** reply language is set to `both`, it produces a natural mix of the language being taught and the student's native language (Japanese script is inline-annotated with romanization) instead of a plain single-language reply — see [API Contract](#api-contract).
